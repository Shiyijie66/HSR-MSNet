...
class PoseHSRNet(nn.Module):
    ...
    def __init__(self, block, layers, cfg, **kwargs):
        ...
        super(PoseHSRNet, self).__init__()
        ...
        #self.deconv_new_layers = self._make_deconv_new_layer(
        #    1,
        #    [1024],
        #    [3],
        #)
    ...
    def _make_deconv_new_layer(self, num_layers, num_filters, num_kernels):
        assert num_layers == len(num_filters), \
            'ERROR: num_deconv_layers is different len(num_deconv_filters)'
        assert num_layers == len(num_kernels), \
            'ERROR: num_deconv_layers is different len(num_deconv_filters)'
        
        logger.info('=> num_layers is {}'.format(num_layers))
        layers = []
    ...
    def _make_deconv_layer(self, num_layers, num_filters, num_kernels):
        ...
        for i in range(num_layers):
            ...
            logger.info('=> kernel is {} padding is {} output_padding is {} inplanes is {} planes is {} '.format(kernel, padding, output_padding, self.inplanes, num_filters[i]))
    ...
    def init_weights(self, pretrained=''):
        if os.path.isfile(pretrained):
            #logger.info('=> init new deconv weights from normal distribution')
            #for name, m in self.deconv_new_layers.named_modules():
            ...
            logger.info('=> loading the part of pretrained model')
...
resnet_spec = {18: (BasicBlock, [2, 2, 2, 2]),
               ...
               110: (Bottleneck, [3, 4, 23, 3, 3]),
               152: (Bottleneck, [3, 8, 36, 3]),
               161: (Bottleneck, [3, 8, 36, 3, 3])}
...
def get_pose_net(cfg, is_train, **kwargs):
    ...
    #logger.info('=> layers is {}'.format(layers));
    model = PoseHSRNet(block_class, layers, cfg, **kwargs)
